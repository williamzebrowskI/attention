...
- booster recovery now has an explicit stage-by-stage attitude policy in `boosterRecovery.js`, and the runtime is locked so `entry-burn` and `landing-burn` stay upright instead of flattening into a Starship-like belly attitude during terminal return.
