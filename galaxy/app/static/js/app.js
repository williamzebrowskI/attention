...
import { computeBoosterEntryThermalState } from "./physics/launch/launchThermalModel.js?v=20260421a";
...
const solarNavigationPanel = document.getElementById("solar-navigation-panel");
const spaceflightPanel = document.getElementById("spaceflight-panel");
const spaceflightPanelContent = document.getElementById("spaceflight-panel-content");
...
const LAUNCH_ENGINE_HUD_SVG_NS = "http://www.w3.org/2000/svg";
const LAUNCH_ENGINE_HUD_CLUSTER_CENTER_X = 88;
const LAUNCH_ENGINE_HUD_CLUSTER_CENTER_Y = 88;
...
const LAUNCH_ENGINE_HUD_SUPER_HEAVY_MAX_RADIUS = LAUNCH_ENGINE_HUD_SUPER_HEAVY_ENGINES.reduce(
...
const LEGEND_SOLAR_NAVIGATION_SECTION_ID = "solar_navigation";
const LEGEND_SPACEFLIGHT_SECTION_ID = "spaceflight_operations";
...
const LAUNCH_PERSISTENCE_SCHEMA_VERSION = 2;
...
let legendBoosterEngineCountSelect = null;
let stagedBoosterEngineCount = 33;
...
function registerLaunchLogDebugHandles() {
  ...
  window.samplePhysicsEnvironment = (context = {}) => {
  ...
  window.getLaunchSnapshot = (bodyId = null, nowMs = Date.now()) => {
    if (!launchController) {
      return null;
    }
    if (bodyId && typeof launchController?.statusSnapshotForBody === "function") {
      return launchController.statusSnapshotForBody(nBodyState, bodyId, nowMs) || null;
    }
    return launchController.statusSnapshot?.(nBodyState, nowMs) || launchController.statusSnapshot?.() || null;
  };
  window.getAppBootstrapState = () => ({
    frontendBuild: FRONTEND_MODULE_VERSION,
    launchFeatureEnabled,
    launchControllerReady: Boolean(launchController),
    nBodyInitialized: Boolean(nBodyState?.initialized),
    bodyCount: Array.isArray(bodies) ? bodies.length : 0,
    positionCount: positionsById?.size || 0,
    runtimeBodyCount: nBodyState?.initialized && Array.isArray(nBodyState?.bodies)
      ? nBodyState.bodies.length
      : 0,
  });
  ...
  window.startSelectedLaunchDebug = async () => startSelectedMissionLaunchFromControls();
  window.startSelectedPadLaunchDebug = async () => startSelectedMissionLaunchFromControls({
    skipEnsureRuntimeReady: true,
  });
...
function sampleEnvironmentRuntime(context = {}) {
  return physicsEnvironmentRuntime.sampleEnvironment(context);
}
...
function initializeLaunchController() {
  ...
  launchController = createLaunchControllerFn({
    ...
    sampleEnvironment: (sampleOptions = {}) => sampleEnvironmentRuntime(sampleOptions),
...
function initializeLegendSectionControls() {
  wireLegendCollapsibleSection(solarNavigationPanel, {
    sectionId: LEGEND_SOLAR_NAVIGATION_SECTION_ID,
    titleSelector: ".body-legend-title",
    title: "Solar system navigation",
    defaultCollapsed: false,
    collapseLabel: "Collapse solar system navigation",
    expandLabel: "Expand solar system navigation",
  });
  wireLegendCollapsibleSection(spaceflightPanel, {
    sectionId: LEGEND_SPACEFLIGHT_SECTION_ID,
    titleSelector: ".body-legend-title",
    title: "Spaceflight operations",
    defaultCollapsed: false,
    collapseLabel: "Collapse spaceflight operations",
    expandLabel: "Expand spaceflight operations",
  });
...
function rebuildBodyLegend() {
  ...
  if (spaceflightPanelContent) {
    spaceflightPanelContent.innerHTML = "";
  }
  ...
  legendBoosterEngineCountSelect = null;
  ...
  const solarFragment = document.createDocumentFragment();
  const spaceflightFragment = document.createDocumentFragment();
  const isSpaceflightBody = (body) => (
    body?.body_type === "spacecraft"
    || isLaunchTelemetryVehicleId(body?.id)
  );
  if (legendBodyCountNode) {
    const solarBodyCount = bodies.filter((body) => !isSpaceflightBody(body)).length;
    legendBodyCountNode.textContent = `${solarBodyCount} tracked`;
  }
  ...
  if (sun) {
    ...
    solarFragment.appendChild(sunGroup.root);
  }
  ...
  solarFragment.appendChild(planetarySystemsSection);
  ...
  solarFragment.appendChild(planetaryGroup.root);
  ...
  if (orphanMoons.length > 0) {
    ...
    solarFragment.appendChild(orphanGroup.root);
  }

  if (launchFeatureEnabled) {
    spaceflightFragment.appendChild(createLegendVehicleViewPanel());
  }

  const spacecraftBodies = bodies
    .filter(isSpaceflightBody)
    .sort((a, b) => sortBySemimajorAxisThenName(a, b));
  if (spacecraftBodies.length > 0) {
    const section = document.createElement("p");
    section.className = "legend-section-title";
    section.textContent = "Spacecraft";
    spaceflightFragment.appendChild(section);
    ...
    spaceflightFragment.appendChild(spacecraftGroup.root);
  }

  bodyLegendList.appendChild(solarFragment);
  if (spaceflightPanelContent) {
    spaceflightPanelContent.appendChild(spaceflightFragment);
  } else {
    bodyLegendList.appendChild(spaceflightFragment);
  }
  applyLegendBodyListCollapsedState();
  updateLegendSelection();
...
function isBoosterCrashInspectionSnapshot(snapshot) {
  if (!snapshot || typeof snapshot !== "object") {
    return false;
  }
  const terminalOutcome = String(snapshot.boosterTerminalOutcome || "").toLowerCase();
  return Boolean(
    snapshot.boosterCrashed
      || snapshot.boosterCrashDynamicsActive
      || snapshot.boosterCrashSettled
      || terminalOutcome === "crashed",
  );
}

function currentBoosterInspectionSnapshot(nowMs = Date.now()) {
  if (!launchFeatureEnabled || !launchController) {
    return null;
  }
  return launchController.statusSnapshotForBody?.(nBodyState, LAUNCH_BOOSTER_BODY_ID, nowMs)
    || launchController.statusSnapshot?.()
    || null;
}

function formatCrashSpeedMS(valueKmS, digits = 1) {
  const numeric = Number(valueKmS);
  return Number.isFinite(numeric) ? `${formatNumber(numeric * 1000, digits)} m/s` : "n/a";
}

function formatCrashAngle(valueDeg, digits = 1) {
  return Number.isFinite(Number(valueDeg)) ? `${formatNumber(valueDeg, digits)}°` : "n/a";
}

function boosterCrashReason(snapshot) {
  return String(
    snapshot?.boosterTerminalReason
      || snapshot?.boosterCrashMode
      || "surface impact",
  ).trim() || "surface impact";
}

function boosterCrashOutcomeLine(snapshot) {
  if (isBoosterCrashInspectionSnapshot(snapshot)) {
    const state = snapshot?.boosterCrashSettled ? "settled wreckage" : "crash dynamics";
    return `crashed (${boosterCrashReason(snapshot)}; ${state})`;
  }
  return snapshot?.boosterLanded
    ? (String(snapshot?.boosterPhase || "").includes("caught") ? "caught" : "landed")
    : "in flight";
}
...
function focusLegendVehicleView(viewMode = "starship") {
  ...
  if (wantsBooster && targetBodyId === LAUNCH_BOOSTER_BODY_ID) {
    const boosterSnapshot = currentBoosterInspectionSnapshot();
    if (isBoosterCrashInspectionSnapshot(boosterSnapshot)) {
      updateLaunchStatusPanel(
        true,
        `Booster wreckage inspection active. Crash reason ${boosterCrashReason(boosterSnapshot)}; engines and guidance are safed.`,
      );
    }
  }
  updateLaunchControls();
...
function createLegendVehicleViewPanel() {
  ...
  const boosterEngineCountLabel = document.createElement("label");
  boosterEngineCountLabel.className = "legend-vehicle-mission-label";
  boosterEngineCountLabel.textContent = "Booster Engines";
  panel.appendChild(boosterEngineCountLabel);

  const boosterEngineCountSelect = document.createElement("select");
  boosterEngineCountSelect.id = "legend-booster-engine-count-select";
  boosterEngineCountSelect.className = "legend-vehicle-mission-select";
  boosterEngineCountSelect.title = "Select the number of Super Heavy engines used for the next booster launch";
  boosterEngineCountLabel.htmlFor = boosterEngineCountSelect.id;
  panel.appendChild(boosterEngineCountSelect);
  legendBoosterEngineCountSelect = boosterEngineCountSelect;
  setupBoosterEngineCountPicker();
...
function updateLaunchMissionControlPanel(snapshot, launchActive) {
  ...
  const boosterCrashInspection = isBoosterCrashInspectionSnapshot(snapshot);
  const boosterMetrics = [
    { label: boosterCrashInspection ? "Wreckage Phase" : "Booster Phase", value: snapshot.boosterPhase || "n/a" },
    { label: boosterCrashInspection ? "Safed Guidance" : "Booster Guidance", value: snapshot.boosterGuidanceMode || "n/a" },
    { label: boosterCrashInspection ? "Wreckage Alt" : "Booster Alt", value: Number.isFinite(Number(snapshot.boosterAltitudeKm)) ? `${formatNumber(snapshot.boosterAltitudeKm, 3)} km` : "n/a" },
    ...
    { label: "Booster Thrust", value: Number.isFinite(Number(snapshot.boosterThrustN)) ? `${formatNumber(Number(snapshot.boosterThrustN) / 1_000_000, 3)} MN${boosterCrashInspection ? " (safed)" : ""}` : "n/a" },
  ...
  if (boosterCrashInspection) {
    boosterMetrics.push(
      { label: "Impact Reason", value: boosterCrashReason(snapshot) },
      { label: "Impact Speed", value: formatCrashSpeedMS(snapshot.boosterImpactSpeedKmS, 1) },
      { label: "Impact V/L", value: `${formatCrashSpeedMS(snapshot.boosterImpactVerticalSpeedKmS, 1)} / ${formatCrashSpeedMS(snapshot.boosterImpactLateralSpeedKmS, 1)}` },
      { label: "Body-Up Align", value: Number.isFinite(Number(snapshot.boosterImpactBodyUpAlignment)) ? formatNumber(snapshot.boosterImpactBodyUpAlignment, 3) : "n/a" },
      { label: "Crash Mode", value: snapshot.boosterCrashSettled ? "settled" : (snapshot.boosterCrashMode || "active") },
      { label: "Crash Tip", value: formatCrashAngle(snapshot.boosterCrashTipAngleDeg, 1) },
      { label: "Crash Angular", value: Number.isFinite(Number(snapshot.boosterCrashAngularSpeedRadS)) ? `${formatNumber(snapshot.boosterCrashAngularSpeedRadS, 3)} rad/s` : "n/a" },
      { label: "Crash Slide", value: formatLaunchSpeedValue(snapshot.boosterCrashSlideSpeedKmS, 4) },
      { label: "Crash Normal", value: formatLaunchSpeedValue(snapshot.boosterCrashNormalSpeedKmS, 4) },
      { label: "Surface Contact", value: snapshot.boosterCrashSurfaceContact ? "yes" : "no" },
      { label: "Body Clearance", value: Number.isFinite(Number(snapshot.boosterCrashClearanceKm)) ? `${formatNumber(Number(snapshot.boosterCrashClearanceKm) * 1000, 2)} m` : "n/a" },
    );
  }
...
function updateLegendVehicleViewButtons() {
  ...
  if (boosterButton) {
    const boosterSnapshot = currentBoosterInspectionSnapshot();
    const boosterCrashed = isBoosterCrashInspectionSnapshot(boosterSnapshot);
    boosterButton.disabled = !viewState.boosterViewAvailable;
    boosterButton.classList.toggle("on", viewState.boosterActive);
    boosterButton.classList.toggle("wreckage", boosterCrashed);
    boosterButton.textContent = boosterCrashed ? "Wreckage View" : "Booster View";
    boosterButton.title = boosterCrashed
      ? `Inspect crashed Super Heavy wreckage (${boosterCrashReason(boosterSnapshot)})`
      : "Track Super Heavy booster";
...
function launchEngineHudFiniteNumberOrNull(value) {
  const numeric = Number(value);
  return Number.isFinite(numeric) ? numeric : null;
}

function launchEngineHudFiniteInteger(value, fallback = 0) {
  const numeric = Number(value);
...
function normalizeLaunchEngineHudIndexArray(values, limit = Infinity) {
...
function initializeLaunchBoosterEngineHud() {
  if (!launchBoosterEngineHudClusterNode || launchBoosterEngineHudNodes.length > 0) {
    return;
  }
  ...
  for (let i = 0; i < LAUNCH_ENGINE_HUD_SUPER_HEAVY_ENGINES.length; i += 1) {
    ...
    circle.setAttribute("class", "launch-booster-engine-hud-node state-idle");
    circle.setAttribute(
      "cx",
      String(LAUNCH_ENGINE_HUD_CLUSTER_CENTER_X + (normalizedX * LAUNCH_ENGINE_HUD_CLUSTER_RADIUS_PX)),
    );
    circle.setAttribute(
      "cy",
      String(LAUNCH_ENGINE_HUD_CLUSTER_CENTER_Y + (normalizedY * LAUNCH_ENGINE_HUD_CLUSTER_RADIUS_PX)),
    );
    ...
    launchBoosterEngineHudClusterNode.appendChild(circle);
    launchBoosterEngineHudNodes.push(circle);
  }
}
...
function resolveLaunchBoosterEngineHudTelemetry(snapshot) {
  ...
  if (attachedStageZero) {
    ...
    const flamePresentIndices = Array.isArray(snapshot.flamePresentIndices)
      ...
      : [];
    const activeCount = launchEngineHudFiniteInteger(snapshot.activeEngineCount, activeIndices.length);
    const desiredCount = launchEngineHudFiniteInteger(snapshot.desiredEngineCount, activeCount);
    return {
      available: true,
      phaseLabel: String(snapshot.boosterPhase || snapshot.phase || "Attached").trim() || "Attached",
      thrustN: launchEngineHudFiniteNumberOrNull(snapshot.thrustN),
      throttle: launchEngineHudFiniteNumberOrNull(snapshot.throttle),
      fuelFraction: boosterFuelFractionFromSnapshot(snapshot),
      activeIndices,
      flamePresentIndices,
      failedIndices: Array.isArray(snapshot.failedEngineIndices) ? snapshot.failedEngineIndices : [],
  ...
  const boosterActiveIndices = Array.isArray(snapshot.boosterActiveEngineIndices)
    ...
    : [];
  const boosterActiveCount = launchEngineHudFiniteInteger(
    snapshot.boosterActiveEngineCount,
    boosterActiveIndices.length,
  );
  const boosterDesiredCount = launchEngineHudFiniteInteger(
    snapshot.boosterDesiredEngineCount,
    boosterActiveCount,
  );
  ...
  if (boosterTelemetryAvailable) {
    return {
      ...
      thrustN: launchEngineHudFiniteNumberOrNull(snapshot.boosterThrustN),
      throttle: launchEngineHudFiniteNumberOrNull(snapshot.boosterThrottle),
      fuelFraction: boosterFuelFractionFromSnapshot(snapshot),
...
function updateLaunchBoosterEngineHud(snapshot) {
  if (!launchBoosterEngineHudNode || launchBoosterEngineHudNodes.length <= 0) {
    return;
  }
  ...
  const fallbackActiveIndices = explicitActiveIndices.length <= 0 && telemetry.activeCount > 0
    ...
    : [];
  const activeSet = new Set(explicitActiveIndices.length > 0 ? explicitActiveIndices : fallbackActiveIndices);
  const explicitFaultedIndices = normalizeLaunchEngineHudIndexArray([
    ...(Array.isArray(telemetry.failedIndices) ? telemetry.failedIndices : []),
    ...(Array.isArray(telemetry.faultedIndices) ? telemetry.faultedIndices : []),
  ], LAUNCH_ENGINE_HUD_SUPER_HEAVY_ENGINES.length);
  const outSet = new Set(selection.desiredIndices.filter((index) => !activeSet.has(index)));
  ...
  const enginesHot = activeSet.size > 0 && (
    (Number.isFinite(telemetry.thrustN) && telemetry.thrustN > 1)
    || (Number.isFinite(telemetry.throttle) && telemetry.throttle > 0.01)
  );
  const fuelFraction = Number.isFinite(Number(telemetry.fuelFraction))
    ? Math.max(0, Math.min(1, Number(telemetry.fuelFraction)))
    : boosterFuelFractionFromSnapshot(snapshot);
  if (launchBoosterEngineHudFuelArcNode) {
...
function normalizeBoosterEngineCountSelection(value, fallback = 33) {
  const fallbackCount = Number.isFinite(Number(fallback))
    ? Math.round(Number(fallback))
    : 33;
  const numeric = Number(value);
  if (!Number.isFinite(numeric)) {
    return clamp(fallbackCount, 1, 33);
  }
  return clamp(Math.round(numeric), 1, 33);
}

function activeSelectedBoosterEngineCount() {
  const selected = Number(legendBoosterEngineCountSelect?.value);
  if (Number.isFinite(selected)) {
    return normalizeBoosterEngineCountSelection(selected, stagedBoosterEngineCount);
  }
  const staged = Number(stagedBoosterEngineCount);
  if (Number.isFinite(staged)) {
    return normalizeBoosterEngineCountSelection(staged);
  }
  const runtimeSelected = Number(launchController?.getBoosterEngineCountSelection?.());
  return normalizeBoosterEngineCountSelection(runtimeSelected, 33);
}

function applyBoosterEngineCountSelection(selectedCount, options = {}) {
  const resolved = normalizeBoosterEngineCountSelection(
    selectedCount,
    launchController?.getBoosterEngineCountSelection?.(),
  );
  const previous = normalizeBoosterEngineCountSelection(stagedBoosterEngineCount, resolved);
  stagedBoosterEngineCount = resolved;
  if (legendBoosterEngineCountSelect && Number(legendBoosterEngineCountSelect.value) !== resolved) {
    legendBoosterEngineCountSelect.value = String(resolved);
  }
  if (!options.silent && previous !== resolved) {
    appendLaunchLogEntry("info", {
      name: "booster_engine_count_changed",
      engineCount: resolved,
      staged: Boolean(launchController?.isActive?.()),
    });
    updateLaunchStatusPanel(
      true,
      launchController?.isActive?.()
        ? `Booster engine count staged for next launch: ${resolved}.`
        : `Booster engine count set: ${resolved}.`,
    );
  }
  return resolved;
}

function setupBoosterEngineCountPicker() {
  if (!legendBoosterEngineCountSelect) {
    return;
  }
  const selectedCount = normalizeBoosterEngineCountSelection(
    launchController?.getBoosterEngineCountSelection?.(),
    stagedBoosterEngineCount,
  );
  legendBoosterEngineCountSelect.innerHTML = "";
  for (let engineCount = 33; engineCount >= 1; engineCount -= 1) {
    const option = document.createElement("option");
    option.value = String(engineCount);
    option.textContent = `${engineCount} ${engineCount === 1 ? "engine" : "engines"}`;
    legendBoosterEngineCountSelect.appendChild(option);
  }
  legendBoosterEngineCountSelect.value = String(selectedCount);
  stagedBoosterEngineCount = selectedCount;
  if (legendBoosterEngineCountSelect.dataset.bound !== "true") {
    legendBoosterEngineCountSelect.addEventListener("change", () => {
      applyBoosterEngineCountSelection(legendBoosterEngineCountSelect.value);
    });
    legendBoosterEngineCountSelect.dataset.bound = "true";
  }
}
...
function persistentLaunchBodyVisualProtected(bodyId) {
  const id = String(bodyId || "").trim();
  return id === String(LAUNCH_BOOSTER_BODY_ID || "");
}
...
async function ensureRuntimeCatalogBody(bodyMeta) {
  ...
  createPromise = ensureRuntimeCatalogBodyView({
    ...
    mountBodyVisual: (visual) => {
      ...
      if (
        persistentLaunchBodyVisualProtected(taggedVisual.id)
        && existingVisual?.root
        && existingVisual !== taggedVisual
      ) {
        disposeBodyVisual(taggedVisual);
        if (!existingVisual.root.parent) {
          scene.add(existingVisual.root);
        }
        purgeDuplicateBodyVisualRoots(taggedVisual.id, existingVisual.root);
        bodyVisuals.set(taggedVisual.id, existingVisual);
        return;
      }
...
async function startSelectedMissionLaunchFromControls(options = {}) {
  const skipEnsureRuntimeReady = options?.skipEnsureRuntimeReady === true;
  if (!skipEnsureRuntimeReady) {
    ...
    if (!launchRuntimeReady.ready) {
      ...
      return { accepted: false, reason: String(launchRuntimeReady.reason || "launch_runtime_unavailable") };
    }
  } else if (!launchController || !nBodyState?.initialized) {
    return { accepted: false, reason: "launch_runtime_not_initialized" };
  }
  const selectedMissionId = legendLaunchMissionSelect?.value
    || launchController.getMissionProfile?.()?.id
    || "";
  const selectedBoosterEngineCount = activeSelectedBoosterEngineCount();
  const missionLaunchMode = selectedMissionLaunchMode();
  ...
  if (missionLaunchAction === "mission_additional_fleet") {
    let launchResult = await requestMissionShipLaunch(
      ...
      selectedMissionId,
      missionLaunchMode,
      { boosterEngineCount: selectedBoosterEngineCount },
    );
    if (
      launchResult?.accepted
    ...
    if (!launchResult?.accepted) {
      ...
      return { accepted: false, action: missionLaunchAction, reason: String(launchResult?.reason || "mission_rejected") };
    }
    await finalizeAcceptedMissionShipLaunch(launchResult, selectedMissionId, missionLaunchMode);
    return { accepted: true, action: missionLaunchAction, missionId: selectedMissionId };
  }
  if (missionLaunchAction === "mission_orbit_inject") {
    ...
    if (launchResult?.pending && launchResult?.autoLaunchPromise) {
      ...
      return { accepted: true, pending: true, action: missionLaunchAction, missionId: selectedMissionId };
    }
    if (!launchResult?.accepted) {
      rejectMissionShipLaunch(
        launchResult,
  ...
  const started = physicsLaunchRuntime.startLaunch(nBodyState, Date.now(), {
    boosterEngineCount: selectedBoosterEngineCount,
    missionIdOverride: selectedMissionId || undefined,
  });
  if (started) {
  ...
  updateLaunchStatusPanel(true);
  return {
    accepted: Boolean(started),
    action: missionLaunchAction,
    missionId: selectedMissionId,
    boosterEngineCount: selectedBoosterEngineCount,
    reason: started ? "" : "controller_rejected_start",
  };
}
...
function setupLaunchControls() {
  ...
  if (launchControlButton) {
    launchControlButton.addEventListener("click", async () => {
      await startSelectedMissionLaunchFromControls();
    });
  }
...
function updateLaunchControls() {
  ...
  const active = Boolean(launchController?.isActive());
  const primaryActive = primaryLaunchActive();
  launchControlButton.textContent = primaryActive ? "Launch +1 Stack" : "Launch";
  launchControlButton.disabled = false;
  launchControlButton.classList.toggle("on", primaryActive);
  launchControlButton.setAttribute("aria-pressed", primaryActive ? "true" : "false");
  ...
  if (legendLaunchMissionSelect) {
    ...
    if (!primaryActive && desiredMissionId && legendLaunchMissionSelect.value !== desiredMissionId) {
  ...
  if (legendBoosterEngineCountSelect) {
    const desiredEngineCount = primaryActive
      ? activeSelectedBoosterEngineCount()
      : normalizeBoosterEngineCountSelection(
        stagedBoosterEngineCount,
        launchController?.getBoosterEngineCountSelection?.(),
      );
    if (Number(legendBoosterEngineCountSelect.value) !== desiredEngineCount) {
      legendBoosterEngineCountSelect.value = String(desiredEngineCount);
    }
    legendBoosterEngineCountSelect.disabled = false;
    legendBoosterEngineCountSelect.title = active
      ? "Select booster engines for the next launch"
      : "Select the number of Super Heavy engines used for launch";
  }
  updateLegendVehicleViewButtons();
}
...
function updateLaunchStatusPanel(force = false, fallbackLine = "") {
  ...
  if (telemetryBodyId === LAUNCH_BOOSTER_BODY_ID && isBoosterCrashInspectionSnapshot(snapshot)) {
    const crashState = snapshot.boosterCrashSettled ? "settled wreckage" : "crash dynamics";
    const impactSpeedLine = formatCrashSpeedMS(snapshot.boosterImpactSpeedKmS, 1);
    const slideLine = formatLaunchSpeedValue(snapshot.boosterCrashSlideSpeedKmS, 4);
    const tipLine = formatCrashAngle(snapshot.boosterCrashTipAngleDeg, 1);
    const contactLine = snapshot.boosterCrashSurfaceContact ? "surface contact" : "contact model";
    launchStatusNode.textContent = `Wreckage inspection | MET ${missionElapsed} | ${crashState} | Impact ${impactSpeedLine} (${boosterCrashReason(snapshot)}) | Tip ${tipLine} | Slide ${slideLine} | ${contactLine} | Engines/guidance safed | ${snapshot.launchSiteName || "Launch Site"}`;
    updateLaunchMissionControlPanel(snapshot, launchActive);
    missionControlScreenController.render(
      snapshot,
      launchActive,
...
function computeReentryHeatTargetForBody(bodyId, launchSnapshot = null) {
  ...
  if (bodyId === LAUNCH_BOOSTER_BODY_ID) {
    const thermalState = computeBoosterEntryThermalState({
      phase: launchSnapshot?.boosterPhase,
      altitudeKm: launchSnapshot?.boosterAltitudeKm ?? kinematics.altitudeKm,
      altitudeAboveTerrainKm: launchSnapshot?.boosterAltitudeAboveTerrainKm,
      dynamicPressurePa: launchSnapshot?.boosterDynamicPressurePa ?? kinematics.dynamicPressurePa,
      airRelativeSpeedKmS: launchSnapshot?.boosterAirRelativeSpeedKmS,
      earthRelativeSpeedKmS:
        launchSnapshot?.boosterEarthRelativeSpeedKmS
        ?? launchSnapshot?.boosterSpeedKmS
        ?? kinematics.speedKmS,
      radialSpeedKmS: kinematics.radialSpeedKmS,
      throttle: launchSnapshot?.boosterThrottle,
    });
    return thermalState.active ? thermalState.heatLevel : 0;
  }
...
function updateLaunchSiteStructureVisuals(deltaSeconds = 0, nowMs = Date.now()) {
  ...
  updateLaunchSiteStructureVisual(structure, {
    ...
    boosterCrashed: snapshot?.boosterCrashed,
    boosterCatchTotalRangeKm: snapshot?.boosterCatchTotalRangeKm,
    boosterCatchLateralRangeKm: snapshot?.boosterCatchLateralRangeKm,
    boosterCatchVerticalErrorKm: snapshot?.boosterCatchVerticalErrorKm,
    boosterCatchLateralSpeedKmS: snapshot?.boosterCatchLateralSpeedKmS,
    boosterCatchVerticalSpeedKmS: snapshot?.boosterCatchVerticalSpeedKmS,
    boosterBodyUpAlignment: snapshot?.boosterBodyUpAlignment,
  });
}
...
function updateLaunchVehicleVisuals(nowMs = Date.now()) {
  ...
  const forceVerticalVisual =
    ...
    || refuelDockingVisual
    || Boolean(dockingLock);
  const actualBodyAxisVisual =
    Boolean(bodyAxisScene)
    && launchActive
    && !earlyTowerClearVisual
    && !visualVerticalHold
    && !refuelDockingVisual
  ...
  if (targetQuaternion) {
    if (dockingLock?.hardLock || visualVerticalHold || actualBodyAxisVisual) {
...
function blendEarlyAscentVisualDirection(targetDirection, upScene, altitudeAboveTerrainKm, stageIndex) {
  ...
  const blendStartKm = Math.max(
    0.08,
    Number(LAUNCH_AUTOPILOT_CONFIG?.towerClearAltitudeKm) || 0.12,
  );
  const blendEndKm = Math.max(
    blendStartKm + 0.4,
    Math.min(
      Number(LAUNCH_AUTOPILOT_CONFIG?.earlyAscentPitchLimitEndAltitudeKm) || 3.0,
      Number(LAUNCH_AUTOPILOT_CONFIG?.pitchKickEndAltitudeKm) || 4.5,
      3.0,
    ),
  );
  if (!(altitudeKm < blendEndKm)) {
    return targetDirection;
  }
  const target = targetDirection.clone();
  const blend = clamp(
    (altitudeKm - blendStartKm) / Math.max(blendEndKm - blendStartKm, 1e-6),
    0,
    1,
  );
...
function launchVisualVerticalHoldActive(snapshot, bodyId = LAUNCH_BODY_ID) {
  ...
  const launchElapsedSecondsRaw = Number(snapshot?.launchElapsedSeconds);
  const launchElapsedSeconds = Number.isFinite(launchElapsedSecondsRaw)
    ? Math.max(0, launchElapsedSecondsRaw)
    : elapsedSeconds;
  ...
  return (
    ...
    && (
      guidanceMode.includes("pad-release")
      || (guidanceMode.includes("tower-clear") && withinTowerClearWindow)
      || guidanceMode.includes("vertical")
      || (
        withinTowerClearWindow
        && (
...
function attachedBoosterVisualSnapshot(snapshot = null) {
  ...
  return {
    ...
    boosterActiveEngineIndices: stage0Active && Array.isArray(snapshot.activeEngineIndices)
      ? [...snapshot.activeEngineIndices]
      : (Array.isArray(snapshot.boosterActiveEngineIndices) ? [...snapshot.boosterActiveEngineIndices] : []),
    boosterActiveEngineCount: stage0Active
      ? (Number(snapshot.activeEngineCount) || 0)
      : (Number(snapshot.boosterActiveEngineCount) || 0),
    boosterDesiredEngineCount: stage0Active
      ? (Number(snapshot.desiredEngineCount) || 0)
      : (Number(snapshot.boosterDesiredEngineCount) || 0),
    boosterFailedEngineIndices: stage0Active && Array.isArray(snapshot.failedEngineIndices)
      ? [...snapshot.failedEngineIndices]
      : (Array.isArray(snapshot.boosterFailedEngineIndices) ? [...snapshot.boosterFailedEngineIndices] : []),
    boosterFaultedEngineIndices: stage0Active && Array.isArray(snapshot.faultedEngineIndices)
      ? [...snapshot.faultedEngineIndices]
      : (Array.isArray(snapshot.boosterFaultedEngineIndices) ? [...snapshot.boosterFaultedEngineIndices] : []),
    boosterFlamePresentIndices: stage0Active && Array.isArray(snapshot.flamePresentIndices)
      ? [...snapshot.flamePresentIndices]
      : (Array.isArray(snapshot.boosterFlamePresentIndices) ? [...snapshot.boosterFlamePresentIndices] : []),
    boosterChamberPressurePaByIndex: stage0Active && Array.isArray(snapshot.chamberPressurePaByIndex)
      ? [...snapshot.chamberPressurePaByIndex]
      : (Array.isArray(snapshot.boosterChamberPressurePaByIndex) ? [...snapshot.boosterChamberPressurePaByIndex] : []),
    boosterExhaustTemperatureKByIndex: stage0Active && Array.isArray(snapshot.exhaustTemperatureKByIndex)
      ? [...snapshot.exhaustTemperatureKByIndex]
      : (Array.isArray(snapshot.boosterExhaustTemperatureKByIndex) ? [...snapshot.boosterExhaustTemperatureKByIndex] : []),
    boosterAvgChamberPressurePa: stage0Active
      ? (Number(snapshot.avgChamberPressurePa) || 0)
      : (Number(snapshot.boosterAvgChamberPressurePa) || 0),
    boosterMaxChamberPressurePa: stage0Active
      ? (Number(snapshot.maxChamberPressurePa) || 0)
      : (Number(snapshot.boosterMaxChamberPressurePa) || 0),
    boosterMaxExhaustTemperatureK: stage0Active
      ? (Number(snapshot.maxExhaustTemperatureK) || 0)
      : (Number(snapshot.boosterMaxExhaustTemperatureK) || 0),
    ...
    boosterSpeedKmS: Number.isFinite(Number(snapshot.boosterSpeedKmS))
      ? Number(snapshot.boosterSpeedKmS)
      : Number(snapshot.speedKmS),
    boosterEarthRelativeSpeedKmS: Number.isFinite(Number(snapshot.boosterEarthRelativeSpeedKmS))
      ? Number(snapshot.boosterEarthRelativeSpeedKmS)
      : Number(snapshot.earthRelativeSpeedKmS ?? snapshot.speedKmS),
    boosterAirRelativeSpeedKmS: Number.isFinite(Number(snapshot.boosterAirRelativeSpeedKmS))
      ? Number(snapshot.boosterAirRelativeSpeedKmS)
      : Number(snapshot.airRelativeSpeedKmS),
...
function updateBoosterVehicleVisuals(nowMs = Date.now()) {
  ...
  const boosterAtmosphereSnapshot = snapshot
    ? {
        ...
        pressurePa: snapshot.boosterPressurePa,
        densityKgM3: snapshot.boosterDensityKgM3,
        dynamicPressurePa: snapshot.boosterDynamicPressurePa,
        speedKmS: snapshot.boosterSpeedKmS,
        earthRelativeSpeedKmS: snapshot.boosterEarthRelativeSpeedKmS ?? snapshot.boosterSpeedKmS,
        airRelativeSpeedKmS: snapshot.boosterAirRelativeSpeedKmS,
      }
    : null;
  const effectNowMs = nowMs;
  const earthAtmosphereContext = earthAtmosphereEffectSceneContext(effectNowMs);
  ...
  const relVelocityKmS = earthVelocityFinite
    ...
    : velocityKmS;
  if (boosterAtmosphereSnapshot && finiteVectorKm(boosterCoordsKm) && finiteVectorKm(earthCoordsKm)) {
    const relPosKm = {
      x: (Number(boosterCoordsKm.x) || 0) - (Number(earthCoordsKm.x) || 0),
      y: (Number(boosterCoordsKm.y) || 0) - (Number(earthCoordsKm.y) || 0),
      z: (Number(boosterCoordsKm.z) || 0) - (Number(earthCoordsKm.z) || 0),
    };
    const relRadiusKm = Math.hypot(relPosKm.x, relPosKm.y, relPosKm.z);
    boosterAtmosphereSnapshot.radialSpeedKmS = relRadiusKm > 1e-9
      ? (
        ((relPosKm.x * (Number(relVelocityKmS.x) || 0))
          + (relPosKm.y * (Number(relVelocityKmS.y) || 0))
          + (relPosKm.z * (Number(relVelocityKmS.z) || 0)))
        / relRadiusKm
      )
      : null;
  }
  ...
  const boosterGuidanceMode = String(snapshot?.boosterGuidanceMode || "").toLowerCase();
  const boosterTerminalOutcome = String(snapshot?.boosterTerminalOutcome || "").toLowerCase();
  const boosterCaughtVisualLock = Boolean(snapshot?.boosterLanded)
    && (
      boosterPhase.includes("caught")
      || boosterGuidanceMode.includes("caught")
  ...
  if (!boosterCaughtVisualLock && !bodyAxisScene && !visualVerticalHold) {
    targetDirection = blendEarlyAscentVisualDirection(
      targetDirection,
      upScene,
...
async function createSpacecraftVisual(body) {
  ...
  if (stack?.state) {
    const initialSnapshot = typeof launchController?.statusSnapshotForBody === "function"
      ...
      : null;
    const stageSnapshot = initialSnapshot;
...
function updateInfoOverlay() {
  ...
  const runtimeCoords = runtimeCoordsOrLiveById(detailBodyId) || null;
  ...
  const sunCoords = runtimeCoordsOrLiveById("sun") || null;
  const hasCoords =
    Boolean(coords) &&
  ...
  const observerLabel =
    observation.mode === OBSERVATION_MODES.SURFACE
      ...
      : "n/a";
  const earthCoordsForAtmosphere = runtimeCoordsOrLiveById("earth") || null;
  ...
  if (isLaunchTelemetryVehicleId(meta.id)) {
    ...
    const boosterCrashInspection = isBoosterSelected && isBoosterCrashInspectionSnapshot(launchSnapshot);
    const selectedVehicleLabel = isBoosterSelected
      ? (boosterCrashInspection ? "Booster Wreckage" : "Booster")
    ...
    const stageTransitionLine = launchSnapshot?.stageTransitionPending
      ? [
        String(launchSnapshot?.stageTransitionKind || "transition"),
        `Wait ${String(launchSnapshot?.stageTransitionWaitReason || "n/a")}`,
        `Hold ${String(launchSnapshot?.stageTransitionHoldMode || "n/a")}`,
        `Anomaly ${String(launchSnapshot?.stageTransitionAnomalyReason || "none")}`,
      ].join(" | ")
      : "n/a";
    const boosterEngineSelectionLine = Number.isFinite(Number(launchSnapshot?.boosterEngineCountSelected))
      ? `${Math.max(1, Math.round(Number(launchSnapshot.boosterEngineCountSelected)))} / 33`
      : "n/a";
    ...
    const boosterThrustLine = Number.isFinite(launchSnapshot?.boosterThrustN)
      ? `${formatNumber(launchSnapshot.boosterThrustN / 1_000_000, 4)} MN @ ${Number.isFinite(launchSnapshot?.boosterThrottle) ? `${formatNumber(launchSnapshot.boosterThrottle * 100, 1)}%` : "n/a"}${boosterCrashInspection ? " (safed)" : ""}`
      : "n/a";
    const boosterMainThrustUpLine = boosterCrashInspection
      ? "safed"
      : (
        Number.isFinite(launchSnapshot?.boosterMainThrustVerticalComponent)
          ? `${formatNumber(launchSnapshot.boosterMainThrustVerticalComponent * 100, 1)}%${launchSnapshot?.boosterMainThrustVerticalFloorActive ? ` floor ${formatNumber((Number(launchSnapshot.boosterMainThrustMinUpComponent) || 0) * 100, 0)}%` : ""}`
          : "n/a"
      );
    const boosterRcsLine = boosterCrashInspection
      ? "safed"
      : (
        launchSnapshot?.boosterRcsActive
    ...
    const boosterLaunchSiteClosingLine = Number.isFinite(launchSnapshot?.boosterLaunchSiteLateralClosingSpeedKmS)
      ...
      : "n/a";
    const boosterCatchRangeLine = Number.isFinite(launchSnapshot?.boosterCatchTotalRangeKm)
      ? `${formatNumber(launchSnapshot.boosterCatchTotalRangeKm, 4)} km`
      : "n/a";
    const boosterCatchLateralSpeedLine = Number.isFinite(launchSnapshot?.boosterCatchLateralSpeedKmS)
      ? `${formatNumber(launchSnapshot.boosterCatchLateralSpeedKmS, 4)} km/s`
      : "n/a";
    const boosterCatchPhaseLine = String(launchSnapshot?.boosterCatchCapturePhase || "").trim() || "n/a";
    const boosterCatchErrorLine = Number.isFinite(launchSnapshot?.boosterCatchCaptureTotalErrorKm)
      ? `${formatNumber(launchSnapshot.boosterCatchCaptureTotalErrorKm, 4)} km`
      : "n/a";
    const boosterCatchSpeedLine = Number.isFinite(launchSnapshot?.boosterCatchCaptureTotalSpeedKmS)
    ...
    const boosterBodyRateDegSLine = finiteVectorKm(launchSnapshot?.boosterBodyAngularRateRadS)
      ...
      : "n/a";
    const boosterCrashInspectionLines = boosterCrashInspection
      ? `
        <p class="line launch-line">Inspection State: ${launchSnapshot?.boosterCrashSettled ? "settled wreckage" : "crash dynamics active"} | Reason: ${boosterCrashReason(launchSnapshot)}</p>
        <p class="line launch-line">Impact Speed: ${formatCrashSpeedMS(launchSnapshot?.boosterImpactSpeedKmS, 1)} | Vertical/Lateral: ${formatCrashSpeedMS(launchSnapshot?.boosterImpactVerticalSpeedKmS, 1)} / ${formatCrashSpeedMS(launchSnapshot?.boosterImpactLateralSpeedKmS, 1)}</p>
        <p class="line launch-line">Impact Body-Up Align: ${Number.isFinite(Number(launchSnapshot?.boosterImpactBodyUpAlignment)) ? formatNumber(launchSnapshot.boosterImpactBodyUpAlignment, 3) : "n/a"} | Tip: ${formatCrashAngle(launchSnapshot?.boosterCrashTipAngleDeg, 1)} | Angular: ${Number.isFinite(Number(launchSnapshot?.boosterCrashAngularSpeedRadS)) ? `${formatNumber(launchSnapshot.boosterCrashAngularSpeedRadS, 3)} rad/s` : "n/a"}</p>
        <p class="line launch-line">Crash Contact: ${launchSnapshot?.boosterCrashSurfaceContact ? "surface contact" : "no"} | Normal: ${formatLaunchSpeedValue(launchSnapshot?.boosterCrashNormalSpeedKmS, 4)} | Slide: ${formatLaunchSpeedValue(launchSnapshot?.boosterCrashSlideSpeedKmS, 4)}</p>
        <p class="line launch-line">Body Clearance: ${Number.isFinite(Number(launchSnapshot?.boosterCrashClearanceKm)) ? `${formatNumber(Number(launchSnapshot.boosterCrashClearanceKm) * 1000, 2)} m` : "n/a"} | Body Above Terrain: ${Number.isFinite(Number(launchSnapshot?.boosterCrashBodyAboveTerrainKm)) ? `${formatNumber(Number(launchSnapshot.boosterCrashBodyAboveTerrainKm) * 1000, 2)} m` : "n/a"}</p>
      `
      : "";
    ...
    if (isBoosterSelected) {
      selectedVehicleLines = `
        <p class="line launch-line">${boosterCrashInspection ? "Wreckage Phase" : "Phase"}: ${launchSnapshot?.boosterPhase || "n/a"}</p>
        <p class="line launch-line">${boosterCrashInspection ? "Safed Guidance" : "Guidance"}: ${boosterGuidanceMode}</p>
        <p class="line launch-line">Booster Engines: ${boosterEngineSelectionLine}</p>
        ...
        <p class="line launch-line">Catch Frame: ${boosterCatchRangeLine} | Lateral Speed: ${boosterCatchLateralSpeedLine} | Tower Nav: ${boosterTowerNavLine}</p>
        <p class="line launch-line">Catch Capture: ${boosterCatchPhaseLine} | Error: ${boosterCatchErrorLine} | Speed: ${boosterCatchSpeedLine}</p>
        ${boosterCrashInspectionLines}
        <p class="line launch-line">Outcome: ${boosterCrashOutcomeLine(launchSnapshot)}</p>
      `;
      ...
      selectedVehicleLines = `
        ...
        <p class="line launch-line">Stage Transition: ${stageTransitionLine}</p>
    ...
    const launchTelemetryContent = `
      ...
      <p class="line">Runtime Build: ${FRONTEND_MODULE_VERSION}</p>
  ...
  const eclipseLine =
    meta.id !== "sun"
      ...
      : "";
  const launchSnapshotBoosterCrashed = isBoosterCrashInspectionSnapshot(launchSnapshot);
  const launchPhysicsLine = launchSnapshot
    ? `<p class="line launch-line">Launch Phase: ${launchSnapshot.phaseLabel || launchSnapshot.phase || "n/a"}</p>
       ...
       <p class="line launch-line">${launchSnapshotBoosterCrashed ? "Wreckage Phase" : "Booster Phase"}: ${launchSnapshot.boosterPhase || "n/a"} | ${launchSnapshotBoosterCrashed ? "Safed Guidance" : "Guidance"}: ${launchSnapshot.boosterGuidanceMode || "n/a"} | Outcome: ${boosterCrashOutcomeLine(launchSnapshot)}</p>
       ...
       <p class="line launch-line">Booster Catch Frame: ${boosterCatchRangeLine} | Lateral Speed: ${boosterCatchLateralSpeedLine} | Tower Nav: ${boosterTowerNavLine}</p>
       <p class="line launch-line">Booster Capture: ${boosterCatchPhaseLine} | Error: ${boosterCatchErrorLine} | Speed: ${boosterCatchSpeedLine}</p>
       ...
       <p class="line launch-line">Booster Thrust: ${Number.isFinite(launchSnapshot.boosterThrustN) ? `${formatNumber(launchSnapshot.boosterThrustN / 1_000_000, 4)} MN${launchSnapshotBoosterCrashed ? " (safed)" : ""}` : "n/a"} @ ${Number.isFinite(launchSnapshot.boosterThrottle) ? `${formatNumber(launchSnapshot.boosterThrottle * 100, 1)}%` : "n/a"}</p>
       <p class="line launch-line">Booster RCS: ${launchSnapshotBoosterCrashed ? "safed" : (launchSnapshot.boosterRcsActive ? `active (${formatNumber((Number(launchSnapshot.boosterRcsAuthority) || 0) * 100, 1)}%)` : "off")} | Jets: ${launchSnapshotBoosterCrashed ? "safed" : (Array.isArray(launchSnapshot.boosterRcsJets) && launchSnapshot.boosterRcsJets.length > 0 ? launchSnapshot.boosterRcsJets.join(", ") : "n/a")}</p>
