...
async function main() {
  ...
  try {
    ...
    assert(
      ...
      "moon_orbit_inject_explicit_seed_async_launch_lock: expected a ready direct-inject seed",
    );
    const launch = await controller.launchMissionShipAsync(
      ...
      {
        mode: "orbit_inject",
        moonDepartureWindowSeed: warmed.solution,
      },
    );
    assert(
      launch?.accepted,
      `moon_orbit_inject_explicit_seed_async_launch_lock: launch rejected (${launch?.reason || "unknown"})`,
    );
    console.log("PASS moon-orbit-inject-explicit-seed-async-launch-lock");
  } finally {
