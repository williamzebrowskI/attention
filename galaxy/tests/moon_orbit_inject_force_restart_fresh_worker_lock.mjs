...
async function main() {
  ...
  try {
    ...
    globalThis.Worker = class MockWorker {
      ...
      postMessage(message) {
        queueMicrotask(() => {
          const onMessage = this.handlers.get("message");
          if (typeof onMessage === "function") {
            onMessage({
              data: {
                requestId: Number(message?.requestId) || 1,
    ...
    const response = await controller.warmMoonOrbitInjectLaunchSolve(
      state,
    ...
    assert(workerCount === 1, `expected one fresh worker, got ${workerCount}`);
    assert(!response?.error, `expected fresh worker solve success, got ${response?.error || "unknown"}`);
    assert(
      response?.solution?.valid && response?.solution?.ready && response?.solution?.corridorAccepted,
      "expected accepted solution from fresh worker",
    );
    console.log("PASS moon-orbit-inject-force-restart-fresh-worker-lock");
  } finally {
