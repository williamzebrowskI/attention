...
async function main() {
  ...
  assert(
    !warmed?.error,
    `moon_orbit_inject_warm_solve_fallback: warm solve errored (${warmed?.error || "unknown"})`,
  );
  assert(
    warmed?.solution?.valid && warmed?.solution?.ready && warmed?.solution?.corridorAccepted,
    "moon_orbit_inject_warm_solve_fallback: expected a ready direct-inject solution",
  );
  ...
  assert(
    launch?.accepted,
    `moon_orbit_inject_warm_solve_fallback: async launch rejected (${launch?.reason || "unknown"})`,
  );

  console.log("PASS moon-orbit-inject-warm-solve-fallback");
}
