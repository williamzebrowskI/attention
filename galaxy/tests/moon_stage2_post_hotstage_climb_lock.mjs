...
function main() {
  ...
  assert(started, "moon_stage2_post_hotstage_climb: startLaunch rejected");

  let nowMs = NOW_MS;
  let stage2StartStep = -1;
  let stage2StartAltitudeKm = Number.NaN;
  let stage2PeakAltitudeKm = -Infinity;
  let guidanceAtPeak = "";
  ...
  for (let step = 0; step < MAX_STEPS; step += 1) {
    ...
    if (Number(snapshot?.stageIndex) >= 1) {
      if (stage2StartStep < 0) {
        stage2StartStep = step;
        stage2StartAltitudeKm = altitudeKm;
      }
      if (altitudeKm > stage2PeakAltitudeKm) {
        stage2PeakAltitudeKm = altitudeKm;
        guidanceAtPeak = String(snapshot?.guidanceMode || "");
      }
      if (step >= stage2StartStep + 70 && altitudeKm < (stage2PeakAltitudeKm - 10)) {
        throw new Error(
          `moon_stage2_post_hotstage_climb: stage 2 lost climb after hotstage (alt ${altitudeKm} km, peak ${stage2PeakAltitudeKm} km, guidance ${snapshot?.guidanceMode})`,
        );
      }
      if (altitudeKm >= 140) {
        break;
  ...
  assert(stage2StartStep >= 0, "moon_stage2_post_hotstage_climb: stage 2 never started");
  assert(
    stage2StartAltitudeKm >= 60 && stage2StartAltitudeKm <= 90,
    `moon_stage2_post_hotstage_climb: expected stage 2 to start near hotstage altitude, got ${stage2StartAltitudeKm}`,
  );
  assert(
    stage2PeakAltitudeKm >= 140,
    `moon_stage2_post_hotstage_climb: expected stage 2 to climb past 140 km, got ${stage2PeakAltitudeKm}`,
  );
  assert(
    guidanceAtPeak.includes("high-orbit-climb") || guidanceAtPeak.includes("high-orbit-insertion"),
    `moon_stage2_post_hotstage_climb: expected high-orbit climb/insertion guidance, got ${guidanceAtPeak}`,
  );

  console.log("PASS moon-stage2-post-hotstage-climb-lock");
}
