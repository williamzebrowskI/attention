...
function main() {
  ...
  const terrainAnchoredSite = {
    name: "Terrain Anchor Test Site",
    latitudeDeg: 35.0,
    longitudeDeg: -106.0,
    altitudeKm: 0,
  };
  try {
    setLaunchSite(terrainAnchoredSite);

    const terrainHeightKm = terrainHeightKmAtLatLon(
      terrainAnchoredSite.latitudeDeg,
      terrainAnchoredSite.longitudeDeg,
    );
    assert(
      terrainHeightKm > 0.2,
      `expected nontrivial terrain height at test site, got ${terrainHeightKm}`,
    );

    const state = makeState();
    const controller = createHarness();
    controller.setMissionProfile(LAUNCH_MISSION_IDS.MOON_ORBIT_RETURN);
    controller.statusSnapshot(state);

    const repairedBody = state.dynamicBodies.get(LAUNCH_BODY_ID);
    const earthState = state.staticSources.get("earth");
    assert(repairedBody && earthState, "expected repaired launch body and Earth state");
    ...
    const terrainSurface = surfacePointRelativeKmAtLatLon(
      terrainAnchoredSite.latitudeDeg,
      terrainAnchoredSite.longitudeDeg,
      earthAxes(),
      { includeTerrain: true },
    );
    const ellipsoidSurface = surfacePointRelativeKmAtLatLon(
      terrainAnchoredSite.latitudeDeg,
      terrainAnchoredSite.longitudeDeg,
      earthAxes(),
    ...
    assert(
      ...
      `expected terrain-anchored pad distance ${expectedTerrainAnchoredDistanceKm}, got ${repairedEarthDistanceKm}`,
    );
    assert(
      (repairedEarthDistanceKm - expectedEllipsoidDistanceKm) >= 0.2,
      `expected repaired pad to sit above ellipsoid-only surface, got delta ${repairedEarthDistanceKm - expectedEllipsoidDistanceKm}`,
    );

    console.log("PASS launch-pad-terrain-anchor-lock");
  } finally {
